...
class SentenceAligner(object):
	...
	def get_batch_word_aligns(self, src_sents: List[str], trg_sents: List[str], batch_size=100) -> Dict[str, List]:
		"""
			Generates word alignments for a batch of sentence pairs.
		"""
		if len(src_sents) != len(trg_sents):
			LOG.warning("Mismatch in Nnmber of source and target sentences.")
			raise ValueError('Mismatch in corpus lengths.')

		words_tokens = []
		for sent_id in range(len(src_sents)):
			l1_tokens = [self.embed_loader.tokenizer.tokenize(word) for word in src_sents[sent_id].split()]
			l2_tokens = [self.embed_loader.tokenizer.tokenize(word) for word in trg_sents[sent_id].split()]
		...
		ds = [(idx, src_sents[idx], trg_sents[idx]) for idx in range(len(src_sents))]
		...
		for batch_id, batch_sentences in enumerate(data_loader):
			...
			for in_batch_id, sent_id in enumerate(batch_sentences[0].numpy()):
				...
				raw_aligns = {x: [] for x in self.matching_methods}
				# b2w_aligns = {x: set() for x in self.matching_methods}
				aligns.append({x: set() for x in self.matching_methods})

				for i in range(len(vectors[0])):
					for j in range(len(vectors[1])):
						for ext in self.matching_methods:
							if all_mats[ext][i, j] > 0:
								raw_aligns[ext].append((i, j))
								if self.token_type == "bpe":
									aligns[sent_id][ext].add(
										(sentences_b2w_map[sent_id][0][i], sentences_b2w_map[sent_id][1][j])
										)
								else:
									aligns[sent_id][ext].add((i, j))

				for ext in aligns[sent_id]:
					aligns[sent_id][ext] = sorted(aligns[sent_id][ext])
